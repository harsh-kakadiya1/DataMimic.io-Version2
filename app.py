...
# NEW: Import Gemini API helper from synthetic_data
from core.synthetic_data import generate_content_with_gemini
...
# NEW API endpoint for AI-powered custom column data generation
@app.route('/api/generate_ai_column_values', methods=['POST'])
def generate_ai_column_values_api():
    try:
        data = request.get_json()
        col_type = data.get('col_type')
        prompt_description = data.get('prompt_description')
        num_records_for_ai_requested = data.get('num_records', 100) # num_records from client (total)

        if not col_type or not prompt_description:
        ...
        num_records_for_ai_actual_query = min(num_records_for_ai_requested, max_ai_generation_limit)

        # Craft a more specific prompt for Gemini
        system_prompt = (
            f"You are a data generation assistant. Generate a comma-separated list of {num_records_for_ai_actual_query} items "
            f"based on the user's description and specified data type. "
            f"Ensure the output is strictly a comma-separated list of values, with no leading/trailing text, "
            f"introductions, numbering, or explanations. Just the raw values."
        )
        
        type_hint = ""
        if col_type == 'Integer':
            type_hint = "Provide integers."
        elif col_type == 'Float':
            type_hint = "Provide numbers with up to two decimal places."
        elif col_type == 'Categorical':
            type_hint = "Provide varied short text values (e.g., categories, names, statuses)."
        elif col_type == 'Date':
            type_hint = "Provide dates in YYYY-MM-DD format."
        elif col_type == 'Boolean':
            type_hint = "Provide true or false values."
        elif col_type == 'String':
            type_hint = "Provide varied short text strings."


        user_prompt = f"Data Type: {col_type}. {type_hint} Description: {prompt_description}"

        api_key = app.config.get('GEMINI_API_KEY')
        if not api_key:
            return jsonify({'success': False, 'message': 'Gemini API key is not configured on the server. Please set GEMINI_API_KEY in your .env file.'}), 500

        # Call the Gemini API helper
        ai_response_text = generate_content_with_gemini(system_prompt + "\n" + user_prompt, api_key)

        if "AI Generation Failed" in ai_response_text:
            return jsonify({'success': False, 'message': ai_response_text}), 500
        if ai_response_text == "Error: No text generated by AI.":
            return jsonify({'success': False, 'message': "Gemini did not generate any text for the given prompt."}), 500


        # Attempt to parse the comma-separated list
        raw_values = [v.strip() for v in ai_response_text.split(',') if v.strip()]
        
        # Basic type conversion attempt for Python list
        processed_values = []
        for val in raw_values:
            if col_type == 'Integer':
                try:
                    processed_values.append(int(float(val))) 
                ...
                processed_values.append(val.lower() == 'true' or val.lower() == 'yes')
            elif col_type == 'Date':
                try:
                    ...
                    processed_values.append(None)
            else: # String, Categorical, other
                processed_values.append(val)

        return jsonify({
            'success': True,
            'message': f'{len(processed_values)} AI-generated values obtained!',
            'generated_values': processed_values,
            'num_records_generated_by_ai': len(processed_values) # Number of actual items AI returned
        })

    except Exception as e:
        app.logger.error(f"Error during AI column value generation: {e}", exc_info=True)
        return jsonify({'success': False, 'message': f'An unexpected error occurred during AI generation: {str(e)}'}), 500
...
def generate_data_api():
    ...
    try:
        ...
        ai_custom_columns_definitions = data.get('aiCustomColumns', [])
        ...
        if not isinstance(ai_custom_columns_definitions, list):
             return jsonify({'success': False, 'message': 'Invalid format for AI custom columns. Expected a list.'}), 400
        ...
        df_generated = generate_synthetic_dataframe(
            ...
            ai_custom_columns_definitions=ai_custom_columns_definitions
        )
        ...
        for col in preview_df.columns:
            if pd.api.types.is_numeric_dtype(preview_df[col]):
                ...
                preview_df[col] = preview_df[col].dt.isoformat(timespec='minutes').replace({pd.NaT: None})
            else:
        ...
        if file_format == 'csv':
            ...
            for col in df_temp.columns:
                if pd.api.types.is_numeric_dtype(df_temp[col]):
                    ...
                    df_temp[col] = df_temp[col].dt.isoformat(timespec='minutes').replace({pd.NaT: None})
                else:
